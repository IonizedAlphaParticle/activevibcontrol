...
where you are on that curve. Near the dead zone the same gains barely do anything;
...
in simulation, so no hardware needed.
"""
...
def simulate(kp, kd, gain_authority, x0=5.0, t_end=3.0, dt=0.01):
    ...
    # steady-state error: how far from zero the system parks at the end
    ss_error = abs(np.mean(x[-int(0.3 / dt):])) / abs(x0)
    return settle, overshoot, ss_error


def cost(settle, overshoot, ss_error):
    # penalize steady-state error hardest -- a controller that doesn't return
    # to zero is useless no matter how little it overshoots
    return settle + 2.0 * overshoot + 8.0 * ss_error
...
def best_gains_for(authority):
    ...
    for kp in np.linspace(2, 60, 12):
        for kd in np.linspace(0.5, 20, 12):
            s, o, e = simulate(kp, kd, authority)
            c = cost(s, o, e)
...
if __name__ == "__main__":
    ...
    for ocr in [500, 650, 800, 950]:
        ...
        sf, of, ef = simulate(fixed_kp, fixed_kd, auth)
        kp, kd = model.predict([[ocr, pwm_to_field(ocr), auth]])[0]
        sa, oa, ea = simulate(kp, kd, auth)
        print(f"  OCR {ocr}:  FIXED settle={sf:.2f}s ov={of:.2f} sserr={ef:.2f}  |  "
              f"ADAPTED settle={sa:.2f}s ov={oa:.2f} sserr={ea:.2f}")
